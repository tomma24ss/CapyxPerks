...
from models import User, CreditLedger, Product, ProductVariant, InventoryLot, Order, OrderItem, CreditType, OrderStatus, UserRole
from schemas import OrderCreate, CreditGrant
...
def get_user_balance(db: Session, user_id: int) -> float:
    ...
    result = db.query(func.sum(CreditLedger.amount)).filter(CreditLedger.user_id == user_id).scalar()
    return float(result or 0.0)


def grant_credits(db: Session, user_id: int, amount: float, description: str, order_id: int = None):
...
def deduct_credits(db: Session, user_id: int, amount: float, description: str, order_id: int):
...
def process_order(db: Session, user: User, order_data: OrderCreate):
    ...
    for item in order_data.items:
        ...
        # Check inventory
        inventory = db.query(InventoryLot).filter(InventoryLot.variant_id == item.variant_id).first()
    ...
    for item_data in items_to_add:
        ...
        inventory = db.query(InventoryLot).filter(InventoryLot.variant_id == item_data["variant_id"]).first()
...
def reject_order(db: Session, order_id: int, reason: str = None):
...
def check_and_reserve_inventory(db: Session, variant_id: int, quantity: int) -> bool:
    ...
    inventory = db.query(InventoryLot).filter(InventoryLot.variant_id == variant_id).first()
